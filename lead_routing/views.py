...
def get_pred(data):
    ...
    try:
        model = joblib.load(str(settings.BASE_DIR)+'/models/model_'+str(pid)+'.pkl')
    ...
    cat_feat = ['region','move_from_type','move_to_type','move_size','user_id','page_url','move_date_month',
                'move_date_week']
    for n in cat_feat:
        globals()[n] = joblib.load(str(settings.BASE_DIR)+'/objects/'+str(pid)+'/'+str(n)+'.pkl')
    ...
    train_cols = joblib.load(str(settings.BASE_DIR)+'/objects/'+str(pid)+'/columns.pkl')
...
# def load_dfs(in_id):
#     df_leads = pd.read_csv(str(settings.BASE_DIR)+"/source_data/moving_leads (4).csv")
#     df_pings = pd.read_csv(str(settings.BASE_DIR)+"/source_data/ping_attempts.csv")
#     #df_posts = pd.read_csv(str(settings.BASE_DIR)+"/source_data/post_attempts (1).csv")
...
#
#
#     files = [f for f in listdir(str(settings.BASE_DIR)+'/models/') if isfile(join(str(settings.BASE_DIR)+'/models/', f))]
...
#         try:
#             globals()['pipe_'+str(pid)] = joblib.load(str(settings.BASE_DIR)+'/models/model_'+str(pid)+'.pkl')
#             for n in cat_feat:
#                 globals()[n+'_'+str(pid)] = joblib.load(str(settings.BASE_DIR)+'/objects/'+str(pid)+'/'+str(n)+'.pkl')
