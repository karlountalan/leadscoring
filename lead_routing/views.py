...
proj_dir = str(settings.BASE_DIR)
...
def get_pred(data):
    ...
    return JsonResponse({'estimated_payout':'pid'}, safe=True)
    ...
    cat_feat = ['region','move_from_type','move_to_type','move_size','user_id','page_url','move_date_month',
                'move_date_week']
    for n in cat_feat:
        globals()[n] = joblib.load(proj_dir+'/objects/'+str(pid)+'/'+str(n)+'.pkl')
    ...
    train_cols = joblib.load(proj_dir+'/objects/'+str(pid)+'/columns.pkl')
